...
`).join("");n.insertAdjacentHTML("beforeend",t);const s={captionsData:"alt"};let i=new b(".gallery a",s);i.on("show.simplelightbox",function(){}),i.refresh()}function E(){const r=`<p class= "end-message">"We're sorry, but you've reached the end of search results."</p>`;n.insertAdjacentHTML("afterend",r)}const h=document.querySelector(".load-more-btn"),y=document.querySelector(".loader");function g(){return y.classList.add("hidden")}function L(){return y.classList.remove("hidden")}function S(){h.style.display="block"}function l(){h.style.display="none"}function H(){const r=document.querySelector(".end-message");r&&r.remove()}let d="",a=1;const u=15;g();l();M.addEventListener("submit",R);async function R(r){if(r.preventDefault(),d=v.value.trim(),a=1,n.innerHTML="",d===""){c.error({title:"Error",message:"Sorry, there are no images matching your search query. Please try again!"}),l();return}H(),L();try{const t=await m(d,a,u),s=t.totalHits;if(n.innerHTML="",t.hits.length===0){n.innerHTML="",c.info({title:"Error",message:"Sorry, there are no images matching your search query. Please try again!"}),l();return}else p(t.hits),S();u*a>=s&&l()}catch{c.error({title:"Error",message:"Sorry, an error occurred while fetching images. Please try again later!"})}finally{g()}}h.addEventListener("click",async()=>{a+=1,L();try{const r=await m(d,a,u),t=r.totalHits;p(r.hits),u*a>=t&&(l(),E());const s=n.firstElementChild.getBoundingClientRect().height;window.scrollBy({top:s*3,behavior:"smooth"})}catch(r){console.error("Error fetching more images:",r),c.error({title:"Error",message:`Error fetching more images: ${r}`})}finally{g()}});
